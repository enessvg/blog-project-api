...
class SiteSettingsResource extends JsonResource
{
    ...
    public function toArray(Request $request): array
    {
        return [
            ...
            'favicon' => $this->favicon ? env('APP_URL').'/storage/'.$this->favicon : env('APP_URL').'/'.env('DEFAULT_IMAGE'),
            'icon' => $this->icon ? env('APP_URL').'/storage/'.$this->icon : env('APP_URL').'/'.env('DEFAULT_IMAGE'),
